class Account
  ...
  def log(strategy, type, count = 1000)
    ...
    account_book.puts "#{time},#{type},#{strategy.average_price_1.round(5)},#{strategy.average_price_2.round(5)},#{price},#{count.round(4)},#{(price * count).round(5)},#{(price * count * 0.002).round(4)},#{usdt_balance.round(5)},#{coin_balance.round(5)},#{(usdt_balance+(price*coin_balance)).round(5)}"
  end
